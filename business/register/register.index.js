define(function () { 
	...
	function Global( vars ) {
		...
		that.layout = function() {
			...
			that.selector.find( "input[name=state]" ).select( {//实例下拉插件
				code: { type: "stat" }
			} );
			...
			cols[ cols.length ] = { title: "登陆账号", name: "loginNo", width: "180px", lockWidth: true };
			cols[ cols.length ] = { title: "名称", name: "loginName", width: "180px", lockWidth: true };
			cols[ cols.length ] = { title: "启用状态", name: "state",width:"60px", lockWidth: true,renderer : function (val, item, rowIndex){
