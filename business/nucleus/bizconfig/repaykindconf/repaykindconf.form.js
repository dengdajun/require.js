	...
	function Global( vars ) {
		...
		that.valdiate = function() {
			...
			that.vars.validator = that.selector.find( "form" ).validate( {
				rules: {
					repayNo: { required: true },
					repayName: { required: true },
...
//					params:{'params[ruleType]':'24400001'}
		...
		handlers.load = function( item ) {
			...
			that.selector.find( ":input[name=repayNo]" ).valChange( item.repayNo ); 
			...
			that.selector.find( ":input[name=repayRemark]" ).valChange( item.repayRemark ); 
		...
		handlers.save = function() {
			...
			that.loading.show(); 
			$.ajax( {
				url: "nucleus/bizconfig/repaykindconf/save",
				type: "POST",
				data: data,
				complete: function() {
					that.loading.hide();
