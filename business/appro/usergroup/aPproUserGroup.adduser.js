define(function () { 
	...
	function Global( vars ) {
		...
		that.layout = function() {
			...
			var config = {
				remote: {
		        	url: "appro/usergroup/notBindUsers",
		            params: {}
		        },
		        multi: true,
		        page: true,
			...
			if ( that.params.item ) {
				...
				config.remote.params["params[groupName]"] = that.params.item.groupName;
		...
		handlers.save = function() {
			...
			that.loading.show(); 
			$.ajax( {
				url: "appro/usergroup/saveBindUsers",
				type: "POST",
				data: {loginNames:loginNames,groupNo:that.selector.find(":input[name='groupNo']").val(),groupName:that.selector.find(":input[name='groupName']").val()},
				complete: function() {
					that.loading.hide();
				},
