define(function () { 
	...
	function Global( vars ) {
		...
		handlers.load = function( item ) {
			...
			that.selector.find( ":input[name=post]" ).valChange( item.post ); 
			that.selector.find( ":input[name=nodeNo]" ).valChange( item.nodeNo ); 
			that.selector.find( ":input[name=nodeNames]" ).valChange( item.nodeNames ); 
			that.selector.find( ":input[name=nodeOrder]" ).valChange( item.nodeOrder ); 
			that.selector.find( ":input[name=isLastNode]" ).valChange( item.isLastNode ); 
			that.selector.find( ":input[name=apprType]" ).valChange( item.apprType ); 
			that.selector.find( ":input[name=apprStep]" ).valChange( item.apprStep ); 
			that.selector.find( ":input[name=ruleId]" ).valChange( item.ruleId ); 
			that.selector.find( ":input[name=disRuleId]" ).valChange( item.disRuleId ); 
