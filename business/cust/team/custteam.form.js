...
//		var moment = require( "moment" );
		...
		that.valdiate = function() {
			...
			that.vars.validator = that.selector.find( "form" ).validate( {
				rules: {
					custTeam: { required: true },
					custTeamName: { required: true },
		...
		handlers.load = function( item ) {
			...
			that.selector.find( ":input[name=custTeam]" ).valChange( item.custTeam ); 
			that.selector.find( ":input[name=custTeamName]" ).valChange( item.custTeamName ); 
			that.selector.find( ":input[name=stat]" ).valChange( item.stat );
			that.selector.find( ":input[name=remark]" ).valChange( item.remark );
			that.selector.find( ":input[name=custTeam]" ).readonly( true );
