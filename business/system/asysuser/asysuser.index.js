...
//			用来解绑网点和组队用户的关系
			that.selector.find( "#tobind" ).bind( "click", function( event ) {
				var items = that.vars.gridVar.selectedRows();
				that.handlers.tobind( items );
				return false;
			} );
...
//			} );
			...
			config.cols = cols = [];
			cols[ cols.length ] = { title: "姓名", name: "userName", width: "100px", lockWidth: true, align:'center'};
			cols[ cols.length ] = { title: "登录名", name: "loginName", width: "100px", lockWidth: true, align:'center' };
			...
			cols[ cols.length ] = { title: "是否启用", name: "status", width: "80px", lockWidth: true,align:'center',
					renderer: function( val, item, rowIndex ) {
						if('13900001'==val){
							return '启用';
						}else if('13900002'==val){
		...
		//用来解绑用户和组/网点的关系
		handlers.tobind = function( items ) {
			if ( items.length != 1 ) {
				return message.error( "请选择一条操作数据。" );
			}
			var item = items[ 0 ]; //获取一条数据
