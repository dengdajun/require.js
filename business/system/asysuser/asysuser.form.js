...
//					loginName: { required: true, minlength:5, maxlength:20},
					...
					isNo: {required: true },
					useDuration: {required: true ,digits: true}
				}
			} );
		};
...
//			} );
			...
			that.selector.find( "input[name=isNo]" ).select( {
				...
				events: {
					change: {
						...
						handler: function (event, val, item) {
							if(val=="13900001"){
								that.selector.find( "input[name=useDuration]" ).readonly(false);
							}else{
								that.selector.find( "input[name=useDuration]" ).val(90);
								that.selector.find( "input[name=useDuration]" ).readonly(true);
							}
						}
					}
				}
		...
		handlers.load = function( item ) {
			...
			that.selector.find( ":input[name=officeTel]" ).valChange( item.officeTel );
			...
			that.selector.find( ":input[name=useDuration]" ).valChange( item.useDuration );
