define(function () { 
	...
	function Global( vars ) {
		...
		that.layout = function() {
			...
			that.selector.find( "input[name=roleCode]" ).select( {//实例下拉插件
				remote: {
					url: "system/asysusergroup/roleCode",
					type: 'POST'
				}
			} );
			that.selector.find( "input[name=orgCode]" ).select( {//实例下拉插件
				remote: {
					url: "system/asysusergroup/orgCode",
					type: 'POST'
				}
			} );
			//提交事件
