...
//		code.cache( "stat,illeflag,oCheaFlg,ilEvFlg" );
		...
		handlers.loadUserTree = function() {
			...
			// 获取taskKey 
			var taskKey = that.params.taskKey;
			var templateKey = that.params.templateKey;
			var nextNodeKey = that.params.nextNodeKey;
			
			var data = [];
			data.push( "params[templateKey]=" + templateKey);
			data.push( "params[nextNodeKey]=" + nextNodeKey);
			data.push( "params[taskKey]=" + taskKey);
						
			that.loading.show();
			$.ajax( {
//				url: "safety/sysinfo/listuserorg",
				url: "flow/flowapr/listuserorg",
				type: "POST",
				data: data.join( "&" ),
