...
//		var moment = require( "moment" );
		...
		that.layout = function() {
			...
			that.selector.find( "input[name=compNo]" ).select( {
				multi: false,
				remote: {
		...
		handlers.load = function( item ) {
			...
			item.mainCtrctNo && that.selector.find( ":input[name=mainCtrctNo]" ).valChange( item.mainCtrctNo );
		...
		handlers.save = function() {
			...
			}else if('26800004' == contractType){//第三方机构
				var compNo = that.selector.find("input[name=compNo]").val();
				if(!compNo || $.trim(compNo) == ''){
					 that.selector.find("input[name=compNo]").validErrorTip("请选择一个第三方机构");
					return;
				}
			}
			...
			that.loading.show(); 
			$.ajax( {
				url: "market/proconfig/relctrct/save",
				type: "POST",
				data: data,
				complete: function() {
					that.loading.hide();
		...
		styctr.init = function (type,value){
			...
			if('26800002' == value){ //资方
				...
				//第三方机构隐藏
				that.selector.find("input[name=compNo]").parent().hide();
				that.selector.find("input[name=compNo]").attr("disabled","disabled");
				...
				that.selector.find("input[name=compNo]").parent().hide();
				that.selector.find("input[name=compNo]").attr("disabled","disabled");
			}else if('26800004' == value){//第三方机构
				//第三方机构显示
				that.selector.find("input[name=compNo]").parent().show();
				that.selector.find("input[name=compNo]").removeAttr("disabled");
				//渠道隐藏
				that.selector.find("input[name=chanNo]").parent().hide();
				that.selector.find("input[name=chanNo]").attr("disabled","disabled");
				//网点隐藏
