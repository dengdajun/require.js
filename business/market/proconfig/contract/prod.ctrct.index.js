define(function () { 
	function Global( vars ) {
		...
		that.layout = function() {
			...
			that.selector.find("#relCtrctAddBtn").bind("click",function(){
				var items = that.vars.gridVar.selectedRows();
				that.handlers.relCtrctAdd(items);
				return false;
			});
			...
			var config = {
		        remote: {
		        	url: "market/proconfig/ctrct/list",
		        ...
		        query: {
		        	isExpand:true,
		        	target: that.selector.find( ".ctrctTmplGrid" )
		        },
		        plugins: [],
		        customEvents: []
			};
			...
			cols[ cols.length ] = { title: "优先级", name: "contractLevel", width: "80px", lockWidth: false };
			...
			that.vars.gridVar = that.selector.find( "#ctrctTmplGrid" ).grid( config );
		};
		...
		handlers.add = function (){
			...
			var item = {"prodNo":$( "#prodNo" ).val()};
			that.modal.open({
				title: "合同|新增",
				url: "market/proconfig/ctrct/form",
				size: "modal-md",
				params: {item:item},
				events: {
		...
		handlers.del = function( items ) {
			...
			that.dialog.confirm( "确定删除选择的[ " + items.length + " ]条数据？", function( event, index ) {
				...
				$.ajax( {
					url: "market/proconfig/ctrct/del",
					type: "POST",
		...
		handlers.moveUpOrDown = function(item,clickBtn){
			...
			that.loading.show();
			$.ajax({
				url: "market/proconfig/ctrct/moveUpOrDown",
				type: "POST",
				data: data,
				complete: function() {
					that.loading.hide();
		...
		//关联合同添加
		handlers.relCtrctAdd = function(items){
			var that = this.global();
			if(!items || items.length==0) return message.error( "请选择一条数据操作" );
			var item = items[0];
			//console.log(item);
			that.page.open({
				title: "合同 | 关联合同",
				url: "market/proconfig/relctrct/"+item.contractType+"/index",
				size: "modal-lg",
				params: {item:item},
				events: {
					hiden: function( closed, data ) {
