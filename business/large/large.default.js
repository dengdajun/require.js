define(function () { 
	...
	function Global( vars ) {
		...
		//缓存码值
		code.cache( "Recheck_Pass,Dec_Cause,Cust_Type" );
		...
		that.valdiate = function() {
			...
			that.vars.validator = that.selector.find( "form" ).validate( {
				rules: {
					...
					custType:{required:true},
					approveAmt:{required:true}
				}
			} );
		};
		//页面布局
		that.layout = function() {
			...
			var applayAmt = that.params.item.loanAmt;
			var custType = that.params.item.custType;
			...
			that.selector.find( "input[name=custType]" ).select( {//实例下拉插件
				code: { type: "Cust_Type" }
			} );
			...
			that.selector.find(":input[name=applayAmt]").valChange(applayAmt);
			that.selector.find(":input[name=custType]").valChange(custType);
			that.selector.find(":input[name=approveAmt]").valChange(applayAmt);
		};
