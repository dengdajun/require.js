define(function () { 
	...
	function Global( vars ) {
		...
		that.loadA = function (){
			var that = this.global();
			if( that.params.loginCode )
				that.handlers.loadA(that.params.loginCode);
		};
		...
		that.valdiate = function() {
			...
			that.vars.validator = that.selector.find( "form" ).validate( {
				rules: {
					loginNo: { required: true ,isUnChinese:true},
					phoneNo: { required: true,isTel:true },
		...
		handlers.load = function( item ) {
			...
			that.selector.find( ":input[name=loginType]" ).valChange( item.loginType ); 
		...
		handlers.loadA = function (loginCode){
			that.selector.find( ":input[name=loginCode]" ).valChange( loginCode ); 
		};
		...
		handlers.save = function() {
			...
			that.loading.show(); 
			$.ajax( {
				url: "operate/channel/fundChan/addLoginUser",
				type: "POST",
				data: data,
				complete: function() {
					that.loading.hide();
