define(function (){
	function Global( vars ){
		...
		base.code.cache( "Is_No,Open_Org,Acc_Type,Acct_Type,Repay_Channel" );
		var that = this; //全局对象
		...
		that.layout = function (){
			...
			that.selector.find( "input[name=settCyc]" ).select( {//实例下拉插件
				code: { type: "SETT_CYC" }
			} );
			
			that.selector.find( "input[name=feeNo]" ).select( {
				remote: {
					url: "nucleus/bizconfig/feeconf/getSelectList?feeBelong=34700001",
					type: 'POST'
				}
			} );
		...
		handlers.load = function( item ) {
			...
			that.selector.find( ":input[name=settCyc]" ).valChange( item.settCyc );
			that.selector.find( ":input[name=feeNo]" ).valChange( item.feeNo );
			
			that.handlers.styctr(item.repayChan , item.acctType);
		};
		...
		handlers.save = function (){
			...
			that.loading.show(); 
			$.ajax( {
				url: "operate/channel/fundbelong/saveFundBelong",
				type: "POST",
				data: data,
				complete: function() {
					that.loading.hide();
		...
		handlers.platInfo = function (){
			//账户归属acctBelong  还款渠道repayChan  账户类型acctType
			var acctBelong = $("#acctBelong").val();
			var repayChan = $("#repayChan").val();
			var acctType = $("#acctType").val();
			if(acctBelong =='28000001' && repayChan!='' && acctType !=''){
				$.ajax( {
					url: "operate/channel/fundbelong/platinfo",
					type: "POST",
					data: {"repayChan":repayChan,"acctBelong":acctBelong,"acctType":acctType},
					complete: function() {
						that.loading.hide();
					},
					success: function( item ) {
						that.selector.find( ":input[name=chanNo]" ).valChange( item.chanNo ); 
						that.selector.find( ":input[name=marNo]" ).valChange( item.platNo ); 
						that.selector.find( ":input[name=marName]" ).valChange( item.platName ); 
						that.selector.find( ":input[name=acctNo]" ).valChange( item.acctNo ); 
						that.selector.find( ":input[name=acctName]" ).valChange( item.acctName ); 
						that.selector.find( ":input[name=acctOrg]" ).valChange( item.acctOrg ); 
						that.selector.find( ":input[name=openBank]" ).valChange( item.openBank ); 
					...
					error: function(e) { 
						...
						if ( !item.id ||(item.Id != "" && (repayChan != item.repayChan || acctType != item.acctType))) {
							that.selector.find( ":input[name=chanNo]" ).valChange(item.chanNo); 
							that.selector.find( ":input[name=marNo]" ).valChange(""); 
							that.selector.find( ":input[name=marName]" ).valChange("" ); 
							that.selector.find( ":input[name=acctNo]" ).valChange( "" ); 
							that.selector.find( ":input[name=acctName]" ).valChange(""  ); 
							that.selector.find( ":input[name=acctOrg]" ).valChange(  ""); 
							that.selector.find( ":input[name=openBank]" ).valChange( ""); 
							that.selector.find( ":input[name=chinapayMerkey]" ).valChange( "" );
							that.selector.find( ":input[name=chinapayPubkey]" ).valChange( "" );
							that.selector.find( ":input[name=certAlias]" ).valChange(  "");
							that.selector.find( ":input[name=chinapayUsername]" ).valChange(""  ); 
							that.selector.find( ":input[name=chinapayPw]" ).valChange( ""); 
							that.selector.find( ":input[name=platNo]" ).valChange( "" );
							that.selector.find( ":input[name=platName]" ).valChange("" );
							that.selector.find( ":input[name=commBank]" ).valChange( "");
							that.selector.find( ":input[name=commAcctNo]" ).valChange(""  );
							that.selector.find( ":input[name=commOpenOrg]" ).valChange(""  );
							that.selector.find( ":input[name=remark]" ).valChange(  "");
						}
					} 
				} );
			}
