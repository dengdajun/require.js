...
//		var moment = require( "moment" );
		...
		that.valdiate = function() {
			...
			that.vars.validator = that.selector.find( "form" ).validate( {
				rules: {
					braCrowdNo: { required: true },
					braCrowdName: { required: true },
		...
		that.layout = function() {
			...
			that.selector.find( ":input[name=ruleId]" ).select( {
				remote: {
					url: "nucleus/bizconfig/sysrule/getSelectList?ruleType=24400008",
					type: 'POST'
				}
			} );
		...
		handlers.load = function( item ) {
			...
			that.selector.find( ":input[name=braCrowdNo]" ).valChange( item.braCrowdNo ); 
			that.selector.find( ":input[name=braCrowdName]" ).valChange( item.braCrowdName ); 
			that.selector.find( ":input[name=ruleId]" ).valChange( item.ruleId );
			that.selector.find( ":input[name=braCrowdDesc]" ).valChange( item.braCrowdDesc );
			that.selector.find( ":input[name=stat]" ).valChange( item.stat );
			that.selector.find( ":input[name=braCrowdNo]" ).readonly( true );
		...
		handlers.save = function() {
			...
			that.loading.show(); 
			$.ajax( {
				url: "operate/channel/crowd/save",
				type: "POST",
				data: data,
				complete: function() {
					that.loading.hide();
