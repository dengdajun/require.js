(function(b) {
	...
	a.config = {
		...
		path : {
			...
			props : {
				text : { name : "text",	label : "路径名称",value : "",	editor : function() { return new a.editors.textEditor() }},
				conditionVal: {name:'conditionVal', label : '流转表达式', value:'', editor: 
					function(){return new a.editors.dynamicSelectPopupEditor( "flow/flowapr/wfRuleView", {
						multi:false,
						labelKey: "label",
						valueKey: "val"} );}},
				remark : { name : "remark",	label : "备注",value : "",	editor : function() { return new a.editors.inputEditor() }}
			}
		},
		tools : {
			attr : {
...
//			console.log(m);
	...
	a.init = function(x, r) {
		var v = b(window).width(), e = b(window).height(), 
...
//		
//		console.log( "props" );
//		console.log( r.restore.props );
		
		if ( typeof a.__config__ == "undefined" ) {
			a.__config__ = a.config;
		}
		a.config = b.extend(true, { width: 2000, height: 2000 }, a.__config__, r);
		
		if ( r && r.restore && r.restore.props && r.restore.props.props ) {
			var props = r.restore.props;
			$.extend( true, a.config.props, props );
		}
		
//		console.log( "config.props" );
//		console.log( a.config.props );
		
		y = Raphael(x, a.config.width, a.config.height);
