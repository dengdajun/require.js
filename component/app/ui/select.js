...
define( [ "app/base", "app/ui/input", "app/ui/code" ], function() {
	...
	//手动调用加载数据
	Select.prototype.load = function( config ) {
		var items = false, params = {};
		if ( typeof arguments[ 0 ] == 'object' ) {
			if ( arguments[ 0 ] instanceof Array ) {
				items = arguments[ 0 ];
			} else if ( arguments[ 0 ].code ) {
				this.opts = $.extend(true, this.opts, __DEFAULT_CODE__, arguments[ 0 ]);
			} else if ( arguments[ 0 ].remote ) {
				this.opts = $.extend(true, this.opts, __DEFAULT_AJAX__, arguments[ 0 ]);
			} else if ( typeof arguments[ 0 ][ opts.root ] instanceof  Array ) {
	...
	$.prototype.select = function( options ) {
		...
		return $( this ).each( function() {
			if ( args.length == 1 ) {
				...
				if ( typeof args[ 0 ] != "string" ) return  new Error( "method (参数2) not a string." );
				
				var select = $( this ).data( "select" );
				if ( typeof select == "undefined" )  return  new Error( "select 未实例化" );
				
				var type = args[ 0 ];
				if ( type == "load" ) {
					select.load( args[ 1 ] );
				} else throw new Error( type + "[method] 没有提供支持的方法" );
			}
